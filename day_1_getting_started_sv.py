# -*- coding: utf-8 -*-
"""Day_1_Getting_Started_SV.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1LBQaIYe2qGeRiR5pdeHaY74cSTugalUT
...
< Some things I can already do with Python are:
-Create basic programs
-Work with lists and dictionaries
-Write functions

Some things I would like to learn to do are:
...
Create a 3 x 3 matrix with all 0s.
Then put 1, 2, 3, 4, 5, 6, 7, 8, 9 in the same matrix in a zigzag fashion. 
...
$Hint:$ You are going to need to use the numpy function **zeros**, and loop over the 2 dimensions.
...
for row in range(3):
    for col in range(3):
        if col == 0 and row == 0:
            ...
            zero_matrix[row, col] = 2
        elif col == 2 and row == 0:
            zero_matrix[row, col] = 3
        elif col == 0 and row == 1:
            zero_matrix[row, col] = 4
        elif col == 1 and row == 1:
            zero_matrix[row, col] = 5
        elif col == 2 and row == 1:
            zero_matrix[row, col] = 6
        elif col == 0 and row == 2:
            zero_matrix[row, col] = 7
        elif col == 1 and row == 2:
            zero_matrix[row, col] = 8
...
Mask out all the elements (with `False`) which are "< 4" in the array
...
< A pandas dataframe is preferable for data that has labels, such as column names. A numpy array is preferable for data that does not have labels. >
...
< A pd.dataframe is a two-dimensional data structure (like a table) that can store data of different types (including characters, integers, floating point values, categorical data, and more) in columns. It is similar to a pd.Series, but it can contain more than one column of data. >
"""

sr = pd.Series([3, -5, 7, 4],  index=['a',  'b',  'c',  'd'])
...
< This will not work because df.loc[] is used to select based on labels and not positions. In order to select based on position, you will want to use df.iloc[].This will not work because df.loc[] is used to select based on labels and not positions. In order to select based on position, you will want to use df.iloc[].>
...
"""# **Upload this Day 1 Colab Notebook to your Github repository under "Day 1" folder. Share your *Reflection* on today's learning in README.md**"""